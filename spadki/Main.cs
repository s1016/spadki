    ...
    public class KlasaSpadki
    {
...

        {
            ...
            try
            {
                ...
                opts.SetRejectMessage("Tylko bloki.");
                ...
                opts.SetRejectMessage("Tylko bloki.");
                opts.AddAllowedClass(typeof(BlockReference), true);
                PromptEntityResult res2 = ed.GetEntity(opts);
                // Do nothing if selection is unsuccessful
                if (res2.Status != PromptStatus.OK)
                    return;

                BlockReference SelRef1 = (BlockReference)tr.GetObject(res.ObjectId, OpenMode.ForRead);
                BlockReference SelRef2 = (BlockReference)tr.GetObject(res2.ObjectId, OpenMode.ForRead);
                string warstwaKota = SelRef2.Layer;

                string attr1 = GetAttribute(tr, SelRef1, "poziom");
                string attr2 = GetAttribute(tr, SelRef2, "poziom");

                attr1 = attr1.Replace('.', ',');
                attr2 = attr2.Replace('.', ',');


                Point3d rz1P = SelRef1.Position;
                Point3d rz2P = SelRef2.Position;
                ...
                double rz2 = double.Parse(attr2);
                Point3d punktwstawiania = myMidPoint(rz1P, rz2P);
                double angle, angle2, atangle, rzN;
                ...
                using (Transaction acTrans = acCurDb.TransactionManager.StartTransaction())
                {
                    ...
                    PromptStringOptions pStrOpts = new PromptStringOptions("\nKtóry blok wstawić - spadek czy kota? (S/K): ");
                    pStrOpts.AllowSpaces = true;
                    PromptResult pStrRes = ed.GetString(pStrOpts);
                    //ed.WriteMessage(pStrRes.StringResult);

                    if (pStrRes.StringResult == "s")
                    {
                        //ed.WriteMessage("wciśnięto s");
                        string warstwa = "!DR_spadki";
                        string fileName = "SPL.dwg";

                        string blockPath = Path.Combine(Path.GetDirectoryName(this.GetType().Assembly.Location), @"bloki\", fileName);
                        //ed.WriteMessage(blockPath);
                        ImportBlocks(blockPath);
                        //ImportBlocks("C:\\SPL.dwg");
                        CreateLayer(db, "!DR_spadki", 0);
                        string attText = spadek.ToString("P2");
                        wstawblok(db, ed, punktwstawiania, angle2, attText, atangle, "SPL", "SP1", warstwa);
                    }

                    else if (pStrRes.StringResult == "k")
                    {
                        //ed.WriteMessage("wciśnięto k");
                        PromptPointResult pPtRes;
                        PromptPointOptions pPtOpts = new PromptPointOptions("");
                        pPtOpts.Message = "\nWskaż miejsce wstawienia: ";
                        pPtRes = ed.GetPoint(pPtOpts);
                        ...
                        ed.WriteMessage("\nrzędna= " + rzN.ToString("N2"));
                        string attText = rzN.ToString("N2");
                        wstawblok(db, ed, punktwstawiania, 0, attText, 0, "Kota2", "Poziom", warstwaKota);
                    }
                    else
                        ed.WriteMessage("nie wybrałeś");
